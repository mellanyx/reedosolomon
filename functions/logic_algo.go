...
// возвращаем список простых чисел меньше N )
...
func russianPeasantMult(x, y, prim, fieldsize int, carryless bool) (result int) {
	for y > 0 {
		if (y & 1) != 0 {
			...
			// y нечетное, тогда x соответствует y
			// (сумма всех x, соответствующих нечетным y, даст конечный результат)
			if carryless {
				result ^= x
			} else {
				result += x
		...
		// ( GF по модулю: если x> = 256, применить модульное сокращение с использованием примитивного полинома
		// если число примитива> 256, напрямую XOR )
...
func forneyAlgo(message, errorPolynomial, locationPolynomial, errPos []int) []int {
	...
	// E сохранит значения, которые необходимо исправить (величину ошибки), чтобы исправить входное сообщение
	...
	for i, location := range locationPolynomial {
		...
		// Вычисление формальной производной многочлена локатора ошибок
		// формальная производная локатора используется в качестве знаменателя для алгоритма Форни,
		// который просто говорит, что i-е значение ошибки задается error_evaluator (gf_inverse (Xi) / error_locator_deriv (gf_inverse (Xi)))
